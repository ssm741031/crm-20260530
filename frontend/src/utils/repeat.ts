...
/** 분 → 일·시·분 분해 (직접 입력 UI용) */
export function splitOffset(min: number): { d: number; h: number; m: number } {
...
/** 일·시·분 → 분 합산 (직접 입력 UI용) */
export function combineOffset(d: number, h: number, m: number): number {
  return Math.max(0, d * 1440 + h * 60 + m);
}

/** 시점 분 → 사람이 읽는 라벨 (프리셋이면 라벨, 아니면 "N일 N시간 N분 전") */
export function reminderLabel(minutesBefore: number): string {
  const preset = REMINDER_PRESETS.find((p) => p.value === minutesBefore);
  if (preset) return preset.label;
  const { d, h, m } = splitOffset(minutesBefore);
  const parts: string[] = [];
  if (d) parts.push(`${d}일`);
  if (h) parts.push(`${h}시간`);
  if (m) parts.push(`${m}분`);
  return (parts.length ? parts.join(" ") : "0분") + " 전";
}
